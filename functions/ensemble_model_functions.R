...
get_all_models_predictions <- function(ngram_matrix, sequences, data_df, model_df, data_path, remove_hmm_files = FALSE) {
  ...
  res <- lapply(unique(data_df[["fold"]]), function(ith_fold) {
    ...
    predict_with_all_models(model_df, ngram_matrix, data_df, sequences, ngram_models, hmm_models, NULL, remove_hmm_files = remove_hmm_files) %>% 
      mutate(train_fold = ith_fold) %>% 
      left_join(select(data_df, c(seq_name, fold)), by = "seq_name")
  }) %>% bind_rows()
  res
} 
