...
tar_load(c(PlastoGram_best_architecture, envelope_traintest_ngram_matrix, envelope_traintest_data_df,
           ngram_matrix_traintest_graphpart, data_df_graphpart))
...
new_PlastoGram_ngram_models_graphpart <- train_ngram_models(PlastoGram_best_architecture, ngram_matrix_traintest_graphpart, data_df_graphpart, filtering_colname = NULL, filtering_term = NULL)
...
ggsave("DPLG_motif_holdout.eps", p, width = 5, height = 5,
...
```{r}
imp <- new_PlastoGram_ngram_models_graphpart[["Nuclear_model"]][["variable.importance"]]
serine_frac <- sapply(names(imp), function(i) str_count(i, "S")/nchar(i))
plot_dat <- data.frame(ngram = names(imp),
                       feature_importance = imp,
                       serine_fraction = serine_frac)
...
## L18 motif ngrams in Membrane model
```{r}
imp_m <- new_PlastoGram_ngram_models_graphpart[["Membrane_model"]][["variable.importance"]]
motifs <- sapply(names(imp_m), function(i) ifelse(nchar(i) != 1, gsub("_", ".", decode_ngrams(i)), i), USE.NAMES = FALSE)
l18 <- "VDPLYPGGSFDPLGLADD"
l18_motifs <- sapply(motifs, function(i) grepl(i, l18), USE.NAMES = FALSE)
dplg_motifs <- sapply(motifs, function(i) grepl(i, "DPLG"), USE.NAMES = FALSE)
...
## L18 motif ngrams in Nuclear membrane model
```{r}
imp_nm <- new_PlastoGram_ngram_models_graphpart[["Nuclear_membrane_model"]][["variable.importance"]]
motifs <- sapply(names(imp_nm), function(i) ifelse(nchar(i) != 1, gsub("_", ".", decode_ngrams(i)), i), USE.NAMES = FALSE)
l18_motifs <- sapply(motifs, function(i) grepl(i, l18), USE.NAMES = FALSE)
dplg_motifs <- sapply(motifs, function(i) grepl(i, "DPLG"), USE.NAMES = FALSE)
plot_dat <- data.frame(ngram = names(imp_nm),
...
ggsave("DPLG_motif_partitioning.eps", p, width = 5, height = 5,
       path = "~/Dropbox/Projekty/BioNgramProjects/PlastoGram/Publication_results/")
```
## L18 motif ngrams in N_E vs. N_TM model

```{r}
imp_tm <- new_PlastoGram_ngram_models_graphpart[["N_E_vs_N_TM_model"]][["variable.importance"]]
motifs <- sapply(names(imp_tm), function(i) ifelse(nchar(i) != 1, gsub("_", ".", decode_ngrams(i)), i), USE.NAMES = FALSE)
l18_motifs <- sapply(motifs, function(i) grepl(i, l18), USE.NAMES = FALSE)
dplg_motifs <- sapply(motifs, function(i) grepl(i, "DPLG"), USE.NAMES = FALSE)
plot_dat <- data.frame(ngram = names(imp_tm),
...
#save(new_PlastoGram_ngram_models, file = "new_PlastoGram_ngram_models.rda")
#load("new_PlastoGram_ngram_models.rda")
lapply(names(new_PlastoGram_ngram_models_graphpart), function(ith_model) {
  x <- importance(new_PlastoGram_ngram_models_graphpart[[ith_model]]) %>% 
    as.data.frame() %>% 
    tibble::rownames_to_column() %>% 
    setNames(c("ngram", "Gini importance")) %>% 
    arrange(`Gini importance`) %>% 
...
```{r}
importance(new_PlastoGram_ngram_models_graphpart[["Nuclear_model"]]) %>% 
  as.data.frame() %>% 
  tibble::rownames_to_column() %>% 
  setNames(c("ngram", "Gini importance")) %>%
  filter(`Gini importance` > 0) %>% 
...
importance(new_PlastoGram_ngram_models_graphpart[["Membrane_model"]]) %>% 
  as.data.frame() %>% 
  tibble::rownames_to_column() %>% 
  setNames(c("ngram", "Gini importance")) %>% 
  filter(`Gini importance` > 0) %>% 
...
importance(new_PlastoGram_ngram_models_graphpart[["Plastid_membrane_model"]]) %>% 
  as.data.frame() %>% 
  tibble::rownames_to_column() %>% 
  setNames(c("ngram", "Gini importance")) %>% 
  filter(`Gini importance` > 0) %>% 
...
importance(new_PlastoGram_ngram_models_graphpart[["Nuclear_membrane_model"]]) %>% 
  as.data.frame() %>% 
  tibble::rownames_to_column() %>% 
  setNames(c("ngram", "Gini importance")) %>% 
  filter(`Gini importance` > 0) %>% 
...
importance(new_PlastoGram_ngram_models_graphpart[["N_E_vs_N_TM_model"]]) %>% 
  as.data.frame() %>% 
  tibble::rownames_to_column() %>% 
  setNames(c("ngram", "Gini importance")) %>% 
  filter(`Gini importance` > 0) %>% 
...
importance(new_PlastoGram_ngram_models_graphpart[["N_E_vs_N_S_model"]]) %>% 
  as.data.frame() %>% 
  tibble::rownames_to_column() %>% 
  setNames(c("ngram", "Gini importance")) %>% 
  filter(`Gini importance` > 0) %>% 
