...
plot_sd_vs_kappa <- function(arch_res, localization) {
  col <- paste0("sd_", localization, "_sens")
  colsens <- paste0("mean_", localization, "_sens")
  ggplot(arch_res, aes(x = get(col), y = mean_kappa, color = get(colsens))) +
    geom_point() +
    xlab(paste0("Standard deviation of ", localization, " sensitivity")) +
    ylab("Mean value of kappa") +
    scale_color_gradientn("Mean sensitivity", colors = c("#ffe96b", "#ff4242", "#630000"))
}
...
We use train-test dataset obtained after homology partitioning to perform 5-fold CV repeated 5 times.
...
plot_sd_vs_kappa(mean_architecture_performance, "N_IM") +
  ggtitle("Number of N_IM sequences = 50")
plot_sd_vs_kappa(mean_architecture_performance, "N_OM") +
  ggtitle("Number of N_OM sequences = 46")
plot_sd_vs_kappa(mean_architecture_performance, "N_TM") +
  ggtitle("Number of N_TM sequences = 192")
plot_sd_vs_kappa(mean_architecture_performance, "N_S") +
  ggtitle("Number of N_S sequences = 287")
plot_sd_vs_kappa(mean_architecture_performance, "N_TL_SEC") +
  ggtitle("Number of N_TL_SEC sequences = 37")
plot_sd_vs_kappa(mean_architecture_performance, "N_TL_TAT") +
  ggtitle("Number of N_TL_TAT sequences = 67")
plot_sd_vs_kappa(mean_architecture_performance, "P_IM") +
  ggtitle("Number of P_IM sequences = 106")
plot_sd_vs_kappa(mean_architecture_performance, "P_TM") +
  ggtitle("Number of P_TM sequences = 1073")
plot_sd_vs_kappa(mean_architecture_performance, "P_S") +
